...
//sh "mvn test -Dcucumber.filter.tags=${tag}"
   ...
   sh "mvn install -Dcucumber.filter.tags=${tag} -DTestingEnvironment='TestingEnvironment=${TestingEnvironment}' -DUseCloudEnv=${UseCloudEnv} -DCloudEnvName=${CloudEnvName} -DCloudEnvName=${CloudEnvName}"
